...
// === ИЗМЕНЕНИЕ: Раздельные размеры для детектора и плоскости реконструкции ===
// Размер детектора (фиксированный, не зависит от входного файла)
const int DETECTOR_SIZE_Y = 256;
const int DETECTOR_SIZE_X = 256;

// Размер плоскости реконструкции (источника) — уменьшен в 4 раза по площади
...
int main()
{
    ...
    // === ИЗМЕНЕНИЕ: Проверяем, что размер детектора соответствует ожидаемому ===
    ...
    if (file_det_x != DETECTOR_SIZE_X || file_det_y != DETECTOR_SIZE_Y) {
        std::cerr << "Warning: Loaded tengram size (" << file_det_y << "x" << file_det_x 
                  << ") does not match expected detector size (" << DETECTOR_SIZE_Y << "x" << DETECTOR_SIZE_X << ")\n";
        // Можно добавить ресайз или завершение, но оставим как есть для совместимости
    }
    
    std::cout << "Real tengram size: " << file_det_y << " x " << file_det_x << "\n";
    ...
    // === ИЗМЕНЕНИЕ: Массив реконструкции имеет размер SOURCE_SIZE ===
    ...
    // === ИЗМЕНЕНИЕ: Проходим по плоскости источника (SOURCE_SIZE), а не детектора ===
    for (int sx = 0; sx < SOURCE_SIZE_Y; ++sx) {
        for (int sy = 0; sy < SOURCE_SIZE_X; ++sy) {
            // Создаём вспомогательную тенеграмму (размер DETECTOR — без изменений)
            ...
            for (int mx = 0; mx < MASK_SIZE_Y; ++mx) {
                for (int my = 0; my < MASK_SIZE_X; ++my) {
                    if (mask[mx][my] == 0) { // 0 = дырка, 1 = закрыто
                        ...
                        int sx_centered = sx - SOURCE_SIZE_Y / 2;  // было: sx - DETECTOR_SIZE_Y / 2
                        int sy_centered = sy - SOURCE_SIZE_X / 2;  // было: sy - DETECTOR_SIZE_X / 2
                        // ==============================================================================
                        ...
                        if (sx == 0 && sy == 0 && !printed_params) {
                            ...
                            std::cout << "  sx_centered (source grid) = " << sx_centered << "\n";
            ...
            // Сохраняем вспомогательную тенеграмму, если это центр (относительно SOURCE_SIZE)
            if (sx == SOURCE_SIZE_X / 2 && sy == SOURCE_SIZE_Y / 2 && !center_aux_saved) {
            ...
            // === Новая логика соответствия (без изменений) ===
            for (int i = 0; i < DETECTOR_SIZE_Y; ++i) {
                for (int j = 0; j < DETECTOR_SIZE_X; ++j) {
    ...
    // === Находим максимум для нормализации при сохранении PGM (размер SOURCE) ===
    double max_recon_value = 0.0;
    for (int i = 0; i < SOURCE_SIZE_Y; ++i) {
        for (int j = 0; j < SOURCE_SIZE_X; ++j) {
            if (source_reconstruction[i][j] > max_recon_value) {
                max_recon_value = source_reconstruction[i][j];
            }
        }
    }
    std::cout << "Max reconstructed value: " << max_recon_value << "\n";

    // Сохраняем результат (текстовый файл с вещественными числами) — размер SOURCE
    ...
    // Создаём PGM-изображение — размер SOURCE (128×128)
    std::ofstream img("./reconstructed_image.pgm");
    img << "P2\n" << SOURCE_SIZE_X << " " << SOURCE_SIZE_Y << "\n255\n";
    for (int i = 0; i < SOURCE_SIZE_Y; ++i) {
        for (int j = 0; j < SOURCE_SIZE_X; ++j) {
            int gray = (max_recon_value > 0) 
                ? static_cast<int>(round(source_reconstruction[i][j] / max_recon_value * 255.0))
                : 0;
            img << gray << " ";
        }
        img << "\n";
    ...
    std::cout << "Reconstructed source saved to reconstructed_source.txt (" << SOURCE_SIZE_Y << "x" << SOURCE_SIZE_X << ")\n";
    std::cout << "Image saved to reconstructed_image.pgm (" << SOURCE_SIZE_Y << "x" << SOURCE_SIZE_X << ")\n";
    std::cout << "First 3 auxiliary tengrams saved as aux_tengram_1.txt/.pgm, aux_tengram_2.txt/.pgm, aux_tengram_3.txt/.pgm (" << DETECTOR_SIZE_Y << "x" << DETECTOR_SIZE_X << ")\n";
