...
int main()
{
    ...
    // === ИЗМЕНЕНИЕ: Массив реконструкции теперь double для накопления значений ===
    std::vector<std::vector<double>> source_reconstruction(DETECTOR_SIZE_Y, std::vector<double>(DETECTOR_SIZE_X, 0.0));
    ...
    for (int sx = 0; sx < DETECTOR_SIZE_Y; ++sx) {
        for (int sy = 0; sy < DETECTOR_SIZE_X; ++sy) {
            ...
            // === ИЗМЕНЕНИЕ: Новая логика соответствия ===
            // Вместо подсчёта совпадений и порога — накапливаем значения из real_tengram
            for (int i = 0; i < DETECTOR_SIZE_Y; ++i) {
                for (int j = 0; j < DETECTOR_SIZE_X; ++j) {
                    // Если в вспомогательной тенеграмме есть проекция И в реальной тенеграмме есть сигнал
                    if (aux_tengram[i][j] > 0 && real_tengram[i][j] > 0) {
                        // Добавляем ЗНАЧЕНИЕ из реальной тенеграммы к источнику (sx, sy)
    ...
    // === ИЗМЕНЕНИЕ: Находим максимум для нормализации при сохранении PGM ===
    ...
    // Сохраняем результат (текстовый файл с вещественными числами)
    ...
    // Создаём PGM-изображение (с нормализацией к 0-255)
    std::ofstream img("./reconstructed_image.pgm");
    img << "P2\n" << DETECTOR_SIZE_X << " " << DETECTOR_SIZE_Y << "\n255\n";
    for (int i = 0; i < DETECTOR_SIZE_Y; ++i) {
        for (int j = 0; j < DETECTOR_SIZE_X; ++j) {
            // Нормализуем: если max > 0, масштабируем к 0-255, иначе 0
